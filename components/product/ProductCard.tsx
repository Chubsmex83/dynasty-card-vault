  ...
  return (
    <MotionCard className="h-full">
      <Link
        ...
        className={cn(
          'group flex h-full flex-col overflow-hidden rounded-2xl bg-panel',
      ...
      >
        ...
        <div className="relative p-3">
          ...
          {soldOut ? (
            <span className="absolute right-5 top-5 rounded-full bg-black/70 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.16em] text-rose-200 backdrop-blur">
              {dict.common.soldOut}
            </span>
          ) : null}
        </div>
        ...
        <div className="flex flex-1 flex-col gap-3 px-4 pb-4 pt-1">
          ...
          <div className="mt-auto flex items-center justify-between gap-2">
            <PriceTag value={product.price} locale={locale} />
            <Button
              variant="gold"
              size="sm"
              onClick={handleAdd}
